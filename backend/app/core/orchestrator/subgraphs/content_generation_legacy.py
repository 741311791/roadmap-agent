...
class ContentGenState(TypedDict):
    ...
    - 仅通过调用时的参数传递数据
    - 使用 Reducer 支持并行累加结果
    - 依赖通过 config["configurable"]["runtime_context"] 传递（不在State中）
    """
    # 输入数据（由主图传入）
    roadmap_id: str
...
def fan_out_concepts(state: ContentGenState) -> Command:
    """
    ...
    # ✅ LangGraph 1.0 正确用法：返回 Command 对象包裹 Send 列表
...
def build_content_generation_subgraph():
    ...
    builder.add_node(
        ...
        retry_policy=LLM_RETRY_POLICY,  # Tutorial 生成使用 LLM
    )
    builder.add_node(
        NodeName.GENERATE_RESOURCE.value,
        generate_resource_for_concept,
        retry_policy=TAVILY_RETRY_POLICY,  # Resource 推荐使用 Tavily 搜索
    )
    builder.add_node(
        NodeName.GENERATE_QUIZ.value,
        generate_quiz_for_concept,
        retry_policy=LLM_RETRY_POLICY,  # Quiz 生成使用 LLM
    )
    
    # 定义边：START -> fan_out -> 动态并行节点 -> END
    builder.add_edge(START, NodeName.FAN_OUT.value)
    
    # ✅ LangGraph 1.0 Command API 正确用法：
    # fan_out 节点返回 Command(goto=[Send(...), ...])，LangGraph 会自动处理并行路由
    # 不需要 add_conditional_edges，Command 会自动将控制流发送到指定的节点
