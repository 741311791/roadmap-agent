...
初次修复时使用了 `content_retry_queued` 作为步骤名称，但这在前端不存在：
...
**前端定义（frontend-next/lib/constants/workflow-steps.ts）：**
- ✅ `content_generation_queued`
- ✅ `content_generation`
- ❌ `content_retry_queued`（不存在）
...
```python
# ✅ 正确：使用前端Content节点对应的步骤
current_step="content_generation"
```

**前端映射逻辑：**
```typescript
// mapToDisplayStep 会将多个子步骤映射到 content_generation 节点
...
// 都映射到 'content_generation' 显示
```

### 修复范围

**3处修改：**
1. API层：`current_step="content_generation"`
2. Worker开始：`step="content_generation"`
3. Worker完成：`step="completed"` 或 `"failed"`

**前后：**
| 位置 | 修复前 | 修复后 |
|------|--------|--------|
| API层 | `content_retry_queued` ❌ | `content_generation` ✅ |
| Worker开始 | `retry_started` ❌ | `content_generation` ✅ |
| Worker完成 | `retry_completed` ❌ | `completed` / `failed` ✅ |
...
| **步骤名称** | 使用前端不存在的步骤 | 使用前端定义的步骤 |
| **前端** | 看不到状态变化 | 实时收到更新，节点正确高亮 |
...
> 
> 2. **前后端步骤名称必须一致：**
>    - 后端发送的 `current_step` 必须在前端 `workflow-steps.ts` 中定义
>    - 否则前端无法找到对应节点，显示异常
>    - 使用前端的 `mapToDisplayStep` 理解映射关系
> 
> 任何遗漏都会导致前后端状态不一致或显示异常。
