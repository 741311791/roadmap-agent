#!/usr/bin/env python3
"""
批量更新 Tavily API Keys 配额脚本（通过官方API查询）

功能：
- 调用管理员接口批量更新 Tavily API Keys 的配额
- 配额数据通过 Tavily 官方 API 实时查询
- 支持从命令行或环境变量读取认证信息
- 显示详细的更新结果
...
使用方法：
    ...
    python scripts/batch_update_tavily_keys.py
...
async def batch_update_tavily_keys(
    ...
    """
    批量更新 Tavily API Keys 配额（通过官方API查询）
    ...
    """
    logger.info(
        "开始批量更新 Tavily Keys（通过官方API查询配额）",
        count=len(keys)
    )
    ...
    async with httpx.AsyncClient(timeout=120.0) as client:
        try:
            response = await client.post(
                f"{base_url}/api/v1/admin/tavily-keys/batch-update",
                json=request_data,
                headers={"Authorization": f"Bearer {token}"},
            )
            ...
            raise Exception(
                f"批量更新失败: HTTP {e.response.status_code} - {e.response.text}"
            )
        except Exception as e:
            logger.error("批量更新异常", error=str(e))
            raise
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="批量更新 Tavily API Keys 配额（通过官方API查询）",
...
注意：
    配额数据会从 Tavily 官方 API 实时查询，无需手动指定配额值。
        """
    )
...
async def main():
    ...
    try:
        ...
        logger.info("")
        logger.info("=" * 60)
        logger.info("步骤 3: 批量更新 Tavily Keys")
        logger.info("=" * 60)
        logger.info(f"待更新 Keys 数量: {len(TAVILY_KEYS)}")
        logger.info("更新方式: 通过 Tavily 官方 API 实时查询配额")
        logger.info("")
        
        result = await batch_update_tavily_keys(
            args.base_url,
            token,
            TAVILY_KEYS
        )
        ...
        # 返回结果
        if failed_count > 0:
            logger.warning(f"部分 Keys 更新失败 ({failed_count}/{len(TAVILY_KEYS)})")
            sys.exit(1)
        else:
            logger.info("所有 Keys 更新成功！")
            sys.exit(0)
