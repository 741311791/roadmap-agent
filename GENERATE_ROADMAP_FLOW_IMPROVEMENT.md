# Generate Roadmap流程改进

## 📋 问题背景

用户询问：
1. 用户在新建路线图时点击generate按钮后可否直接跳转到任务详情页？
2. 任务ID生成时机是什么时候？

---

## 🔍 原流程分析

### 任务ID生成时机

```
用户操作：点击"Generate Roadmap"按钮
    ↓
后端处理：创建任务并立即返回task_id
    ↓
前端接收：onSuccess回调收到response.task_id
    ↓
WebSocket：启动实时监听任务进度
```

**关键点：** task_id在API响应时**立即生成**，不需要等待任务完成。

---

### 原跳转逻辑

```
/new (新建路线图页面)
  ↓ 点击Generate Roadmap
API调用 → 返回task_id
  ↓
停留在/new页面
  ↓ WebSocket监听进度
  ↓ 显示进度条和状态
  ↓
任务完成 (100%)
  ↓ autoNavigate: true
自动跳转 → /roadmap/{roadmap_id}
```

**问题：**
- ❌ 用户必须停留在`/new`页面等待完成
- ❌ 刷新页面会丢失进度显示
- ❌ 无法复制/分享正在生成的任务链接
- ❌ 进度状态和任务详情页重复

---

## ✅ 改进后的流程

### 新跳转逻辑

```
/new (新建路线图页面)
  ↓ 点击Generate Roadmap
API调用 → 返回task_id
  ↓ 立即跳转！
/tasks/{task_id} (任务详情页)
  ↓ WebSocket监听进度
  ↓ 显示横向步进器和日志
  ↓ 用户可点击各个阶段查看日志
  ↓
任务完成
  ↓ 用户点击"Final"阶段
显示RoadmapReadyBanner
  ↓ 点击按钮
手动跳转 → /roadmap/{roadmap_id}
```

**优势：**
- ✅ 用户可以刷新页面不丢失进度
- ✅ 可以复制/分享任务链接给别人查看
- ✅ 统一的任务追踪体验
- ✅ 用户主动查看完成结果（点击Final阶段）
- ✅ 更清晰的工作流可视化

---

## 🔧 代码改动

### 文件：`app/(app)/new/new-roadmap-client.tsx`

#### 1. 移除WebSocket监听（已跳转走）

```diff
- import { useRoadmapGenerationWS } from '@/lib/hooks/websocket/use-roadmap-generation-ws';

- const { connectionType, isConnected } = useRoadmapGenerationWS(taskId, {
-   onComplete: (roadmapId) => {
-     console.log('[Generation] Complete, navigating to:', roadmapId);
-   },
-   onError: (error) => {
-     console.error('[Generation] Error:', error);
-   },
-   autoNavigate: true,
- });
```

#### 2. 添加立即跳转逻辑

```diff
generateRoadmap(request, {
  onSuccess: (response) => {
    console.log('[Generate] Task created:', response.task_id);
    setTaskId(response.task_id);
+   
+   // 立即跳转到任务详情页
+   router.push(`/tasks/${response.task_id}`);
  },
  onError: (error) => {
    console.error('[Generate] Error:', error);
  },
});
```

#### 3. 优化任务恢复逻辑

```diff
// 如果任务还在进行中
if (taskStatus === 'processing' || taskStatus === 'pending' || taskStatus === 'human_review_pending') {
- setTaskId(resumeTaskId);
- setStep('generating');
- // 更新进度状态
- if (status.current_step) {
-   const stepInfo = stepProgress[status.current_step];
-   if (stepInfo) {
-     updateProgress(status.current_step, stepInfo.progress);
-   }
- }
+ console.log('[NewRoadmap] Task in progress, navigating to task detail:', resumeTaskId);
+ router.push(`/tasks/${resumeTaskId}`);
  return;
}
```

#### 4. 移除generating步骤的UI

```diff
- {step === 'generating' && (
-   <Card>
-     <CardContent className="py-16 text-center">
-       {/* 进度显示... */}
-     </CardContent>
-   </Card>
- )}
```

---

## 🎯 用户体验对比

### Before (原流程)
```
用户: 点击Generate
前端: [停留在/new，显示进度条]
      [Loader动画 + 百分比 + 状态文字]
      [WebSocket连接状态]
      等待... (可能2-5分钟)
      [100%完成]
      自动跳转到/roadmap/{id}
用户: "我能刷新吗？" ❌
用户: "我能分享这个链接吗？" ❌
```

### After (新流程)
```
用户: 点击Generate
前端: [立即跳转到/tasks/{task_id}] ✅
      [显示横向步进器]
      [实时更新各阶段状态]
      [可点击查看每个阶段的日志]
用户: "我能刷新页面吗？" ✅ 可以
用户: "我能分享这个链接吗？" ✅ 可以
用户: 等待完成后点击"Final"阶段
前端: [显示RoadmapReadyBanner]
用户: 点击"View Roadmap"按钮
前端: [跳转到/roadmap/{id}]
```

---

## 📊 技术流程图

### 原流程
```
┌──────────┐      ┌─────────┐      ┌──────────┐
│   /new   │─────→│  API    │─────→│   /new   │
│  页面    │Generate│ task_id │      │ 等待完成 │
└──────────┘      └─────────┘      └──────────┘
                                          │
                                          ↓
                                    ┌──────────┐
                                    │ /roadmap │
                                    │   自动   │
                                    └──────────┘
```

### 新流程
```
┌──────────┐      ┌─────────┐      ┌──────────┐
│   /new   │─────→│  API    │─────→│  /tasks  │
│  页面    │Generate│ task_id │ 立即 │ 任务详情 │
└──────────┘      └─────────┘      └──────────┘
                                          │
                                    [用户主动]
                                          ↓
                                    ┌──────────┐
                                    │ /roadmap │
                                    │   点击   │
                                    └──────────┘
```

---

## 🚀 后续优化建议

### 短期
- [ ] 在`/new`页面点击Generate后添加短暂的loading状态
- [ ] 添加toast提示："Task created, redirecting..."
- [ ] 任务列表页面可以看到所有创建的任务

### 长期
- [ ] 支持批量创建多个任务
- [ ] 任务详情页支持取消/重试
- [ ] 添加任务历史记录

---

## ✅ 验证清单

- [x] 点击Generate后立即跳转到`/tasks/{task_id}`
- [x] 任务详情页正常显示进度
- [x] 刷新任务详情页进度不丢失
- [x] URL参数恢复逻辑正常工作
- [x] 完成后点击Final显示RoadmapReadyBanner
- [x] RoadmapReadyBanner跳转链接正确
- [x] 移除了`/new`页面的generating步骤UI
- [x] 无TypeScript/Lint错误

---

*改进完成时间: 2025-12-13*
*改进目标: 提升用户体验，统一任务追踪*










