...
import type { QuizResponse, ResourcesResponse, TutorialWithContent } from '@/types/generated/services';
...
export default function LearningPage() {
  ...
  // Use new Hooks
  const { data: tutorialData, isLoading: tutorialLoading } = useTutorial(roadmapId, conceptId);
  const { data: quizData, isLoading: quizLoading } = useQuiz(roadmapId, conceptId);
  const { data: resourcesData, isLoading: resourcesLoading } = useResources(roadmapId, conceptId);

  // Extract tutorial content
  const tutorial = tutorialData?.full_content || 
    (tutorialData?.status === 'generating' ? '# 教程内容生成中\n\n教程内容正在生成，请稍后刷新页面。' : 
     '# 教程内容暂未生成\n\n该概念的教程内容尚未生成。');

  // Fetch roadmap to find concept details
  useEffect(() => {
    const fetchConcept = async () => {
      try {
        ...
        // Get roadmap to find concept details
  ...
  // Loading state
  const isDataLoading = loading || tutorialLoading || quizLoading || resourcesLoading;
          ...
          <div className="bg-white shadow-sm border border-stone-100 rounded-xl p-12 md:p-16 min-h-[80vh]">
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab}>
                          ...
                          <div className="flex items-center justify-between">
                            <div>
                              ...
                              <p className="text-sage-600">
                                得分：{getQuizScore()} / {quizData.total_questions}
                              </p>
                            </div>
                            <Button
                              variant="outline"
              ...
              <TabsContent value="resources">
                {!resourcesData || resourcesData.resources_count === 0 ? (
                  <div className="text-center py-12">
                  ...
                  <div className="space-y-4">
                    {resourcesData.resources.map((resource: any, index: number) => (
                      <a
