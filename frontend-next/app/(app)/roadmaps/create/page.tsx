...
import type { RoadmapGenerationEvent } from '@/lib/schemas/sse-events';
...
export default function CreateRoadmapPage() {
  ...
  useEffect(() => {
    const loadProfile = async () => {
      ...
      if (!userId) {
        console.error('[CreateRoadmap] No user ID');
          ...
          onEvent: (event: RoadmapGenerationEvent) => {
            ...
            switch (event.type) {
              case 'chunk': {
                ...
                setCurrentPhase(chunkEvent.agent);
                setGenerationPhase(chunkEvent.agent === 'intent_analyzer' ? 'intent_analysis' : 'curriculum_design');
                setPhaseBuffer((prev) => prev + chunkEvent.content);
                appendGenerationBuffer(chunkEvent.content);
                break;
              }
              case 'complete': {
                // Agent completed
              ...
              case 'tutorials_start': {
                const tutorialsStartEvent = event;
                setCurrentPhase('tutorials');
                setGenerationPhase('content_generation');
                setTutorialStatus({ completed: 0, total: tutorialsStartEvent.total_count });
                updateTutorialProgress(0, tutorialsStartEvent.total_count);
                setCurrentBatch({ 
                  index: 0, 
                  total: Math.ceil(tutorialsStartEvent.total_count / tutorialsStartEvent.batch_size) 
                });
                break;
              }
              case 'batch_start': {
                const batchStartEvent = event;
                setCurrentBatch({ index: batchStartEvent.batch_index, total: batchStartEvent.total_batches });
                break;
              }
              case 'tutorial_start': {
                const tutorialStartEvent = event;
                console.log(`Starting tutorial: ${tutorialStartEvent.concept_name}`);
                break;
              }
              ...
              case 'tutorial_error': {
                const tutorialErrorEvent = event;
                console.error(`Tutorial error for ${tutorialErrorEvent.concept_id}:`, tutorialErrorEvent.error);
                break;
              }
              case 'batch_complete': {
                const batchCompleteEvent = event;
                console.log(`Batch ${batchCompleteEvent.batch_index} complete:`, batchCompleteEvent.progress);
                break;
              }
              case 'tutorials_done': {
                const tutorialsDoneEvent = event;
                console.log('All tutorials complete:', tutorialsDoneEvent.summary);
                break;
              }
              case 'done': {
                ...
                // Add to history
                if (doneEvent.roadmap_id) {
                  const eventData = doneEvent as any;
                  const framework = eventData.summary?.framework;
                  addToHistory({
                    roadmap_id: doneEvent.roadmap_id,
                  ...
                  setTimeout(() => {
                    router.push(`/roadmap/${doneEvent.roadmap_id}`);
                  }, 1500);
                }
                break;
              }
              ...
              case 'error': {
                ...
                alert(`Error: ${genericErrorEvent.message}`);
                setIsGeneratingLocal(false);
                setGenerating(false);
                setCurrentPhase('error');
                break;
              ...
              default:
                // Handle progress and step_complete events
                break;
            }
          },
          onComplete: () => {
  ...
  const handleContentPreferenceToggle = (type: string) => {
    setFormData((prev) => {
      const currentPref = prev.content_preference || [];
      return {
        ...prev,
        content_preference: currentPref.includes(type as any)
          ? currentPref.filter((t) => t !== type)
          : [...currentPref, type as any],
      };
    });
  };
                ...
                <Label>内容偏好</Label>
                  <button
                    ...
                    onClick={() => {
                      ...
                      const currentPref = formData.content_preference || [];
                      if (currentPref.length === 4) {
